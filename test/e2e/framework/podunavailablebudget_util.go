...
func (t *PodUnavailableBudgetTester) CreatePub(pub *policyv1alpha1.PodUnavailableBudget) *policyv1alpha1.PodUnavailableBudget {
	...
	_, err := t.kc.PolicyV1alpha1().PodUnavailableBudgets(pub.Namespace).Create(pub)
	gomega.Expect(err).NotTo(gomega.HaveOccurred())
	t.WaitForPubCreated(pub)
	pub, _ = t.kc.PolicyV1alpha1().PodUnavailableBudgets(pub.Namespace).Get(pub.Name, metav1.GetOptions{})
	return pub
}
...
func (t *PodUnavailableBudgetTester) CreateDeployment(deployment *apps.Deployment) {
	...
	_, err := t.c.AppsV1().Deployments(deployment.Namespace).Create(deployment)
	gomega.Expect(err).NotTo(gomega.HaveOccurred())
...
func (t *PodUnavailableBudgetTester) WaitForPubCreated(pub *policyv1alpha1.PodUnavailableBudget) {
	pollErr := wait.PollImmediate(time.Second, time.Minute,
		func() (bool, error) {
			_, err := t.kc.PolicyV1alpha1().PodUnavailableBudgets(pub.Namespace).Get(pub.Name, metav1.GetOptions{})
			if err != nil {
				return false, err
			}
			return true, nil
...
func (t *PodUnavailableBudgetTester) WaitForDeploymentRunning(deployment *apps.Deployment) {
	pollErr := wait.PollImmediate(time.Second, time.Minute*5,
		func() (bool, error) {
			inner, err := t.c.AppsV1().Deployments(deployment.Namespace).Get(deployment.Name, metav1.GetOptions{})
			if err != nil {
				return false, err
			}
...
func (t *PodUnavailableBudgetTester) DeletePubs(namespace string) {
	pubList, err := t.kc.PolicyV1alpha1().PodUnavailableBudgets(namespace).List(metav1.ListOptions{})
	if err != nil {
	...
	for _, pub := range pubList.Items {
		err := t.kc.PolicyV1alpha1().PodUnavailableBudgets(namespace).Delete(pub.Name, &metav1.DeleteOptions{})
		if err != nil {
...
func (t *PodUnavailableBudgetTester) DeleteDeployments(namespace string) {
	deploymentList, err := t.c.AppsV1().Deployments(namespace).List(metav1.ListOptions{})
	if err != nil {
	...
	for _, deployment := range deploymentList.Items {
		err := t.c.AppsV1().Deployments(namespace).Delete(deployment.Name, &metav1.DeleteOptions{})
		if err != nil {
...
func (t *PodUnavailableBudgetTester) DeleteCloneSets(namespace string) {
	objectList, err := t.kc.AppsV1alpha1().CloneSets(namespace).List(metav1.ListOptions{})
	if err != nil {
	...
	for _, object := range objectList.Items {
		err := t.kc.AppsV1alpha1().CloneSets(namespace).Delete(object.Name, &metav1.DeleteOptions{})
		if err != nil {
...
func (t *PodUnavailableBudgetTester) WaitForDeploymentDeleted(deployment *apps.Deployment) {
	pollErr := wait.PollImmediate(time.Second, time.Minute,
		func() (bool, error) {
			_, err := t.c.AppsV1().Deployments(deployment.Namespace).Get(deployment.Name, metav1.GetOptions{})
			if err != nil {
...
func (t *PodUnavailableBudgetTester) WaitForCloneSetDeleted(cloneset *appsv1alpha1.CloneSet) {
	pollErr := wait.PollImmediate(time.Second, time.Minute,
		func() (bool, error) {
			_, err := t.kc.AppsV1alpha1().CloneSets(cloneset.Namespace).Get(cloneset.Name, metav1.GetOptions{})
			if err != nil {
				if errors.IsNotFound(err) {
					return true, nil
				}
