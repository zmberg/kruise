...
func (p *enqueueRequestForPod) updatePod(q workqueue.RateLimitingInterface, old, cur runtime.Object) {
	...
	if !reflect.DeepEqual(newPod.Labels, oldPod.Labels) {
		oldPub := p.getPubForPod(oldPod)
		newPub := p.getPubForPod(newPod)
		if oldPub != nil && newPub != nil && oldPub.Name == newPub.Name {
			control := podunavailablebudget.NewPubControl(newPub)
...
func (p *enqueueRequestForPod) getPubForPod(pod *corev1.Pod) *policyv1alpha1.PodUnavailableBudget {
	var err error
	if len(pod.Labels) == 0 {
		return nil
	}
	...
	if err = p.client.List(context.TODO(), pubList, &client.ListOptions{Namespace: pod.Namespace}); err != nil {
		klog.Errorf("List PodUnavailableBudget failed: %s", err.Error())
		return nil
	}
	...
	return &matchedPubs[0]
}

func isPodAvailableChanged(oldPod, newPod *corev1.Pod, pub *policyv1alpha1.PodUnavailableBudget, control podunavailablebudget.PubControl) (bool, time.Duration) {
	...
	oldReady := control.IsPodConsistentAndReady(oldPod)
	newReady := control.IsPodConsistentAndReady(newPod)
