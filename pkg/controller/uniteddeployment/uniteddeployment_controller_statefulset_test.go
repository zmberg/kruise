...
func TestStsReconcile(t *testing.T) {
	g, requests, _, cancel, mgrStopped := setUp(t)
	defer func() {
		clean(g, c)
		cancel()
		mgrStopped.Wait()
...
func setUp(t *testing.T) (*gomega.GomegaWithT, chan reconcile.Request, context.Context, context.CancelFunc, *sync.WaitGroup) {
	...
	return g, requests, cxt, cancel, mgrStopped
}
